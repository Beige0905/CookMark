...
### 5. 프로젝트 위키 유지 (Wiki Maintenance)

대화 중 다음 중 하나를 발견하면 **즉시** `docs/wiki/` 해당 파일에 기록하세요. 나중에 하겠다고 미루지 마세요.

**기록 대상:**
- 검토했지만 채택하지 않은 접근법과 그 이유
- 디버깅·구현 중 발견한 비자명한 제약사항이나 함정 (DB 동작, 라이브러리 quirk 등)
- 현재 구조가 된 배경과 트레이드오프

**기록 위치:**
- 기능별: `docs/wiki/recipes.md`, `fridge.md`, `cooking-log.md`, `auth.md`
- 전역 제약사항: `docs/wiki/constraints.md`
- 새 기능이 생기면 새 파일 추가 후 `docs/wiki/index.md`에 등록

**기록하지 않는 것:** 공식 설계 결정 (→ `docs/decisions/`), 코드에서 읽을 수 있는 내용

---

### 6. 상업적 이용 안전성 (Commercial Safety)
