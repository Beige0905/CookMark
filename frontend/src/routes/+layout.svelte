			...
			<aside class="hidden w-80 shrink-0 border-r border-stone-100 p-8 lg:sticky lg:top-0 lg:block lg:h-screen">
				<div class="flex flex-col h-full">
					...
					<a href="/" class="flex items-baseline gap-2 mb-12 hover:opacity-80 transition-opacity">
						<span class="text-2xl font-bold tracking-tight text-stone-900">Cookmark</span>
						<span class="text-xs font-medium text-stone-400">나의 레시피 보관함</span>
					</a>
					...
					<nav class="flex flex-col gap-2 mb-12">
						<a
							href="/"
							class="flex items-center gap-3 px-4 py-3 rounded-2xl transition-colors {isActive('/') ? 'bg-[#7C9A7E] text-white' : 'text-stone-500 hover:bg-stone-50'}"
						>
						...
						<a
							href="/recipes"
							class="flex items-center gap-3 px-4 py-3 rounded-2xl transition-colors {isActive('/recipes') ? 'bg-[#7C9A7E] text-white' : 'text-stone-500 hover:bg-stone-50'}"
						>
						...
						<a
							href="/fridge"
							class="flex items-center gap-3 px-4 py-3 rounded-2xl transition-colors {isActive('/fridge') ? 'bg-[#7C9A7E] text-white' : 'text-stone-500 hover:bg-stone-50'}"
						>
						...
						<a
							href="/settings"
							class="flex items-center gap-3 px-4 py-3 rounded-2xl transition-colors {isActive('/settings') ? 'bg-[#7C9A7E] text-white' : 'text-stone-500 hover:bg-stone-50'}"
						>
					...
					<div class="mt-auto space-y-4">
						<div class="rounded-2xl bg-[#F5F2EC] p-5">
							<p class="mb-1 text-xs font-medium text-stone-500 uppercase tracking-wider">총 레시피</p>
							<p class="text-4xl font-bold text-stone-800">12</p>
						</div>
						<div class="rounded-2xl bg-[#EEF3EE] p-5">
							<p class="mb-1 text-xs font-medium text-stone-500 uppercase tracking-wider">이번 달 요리</p>
							<p class="text-4xl font-bold text-stone-800">5<span class="ml-1 text-sm font-normal text-stone-400">회</span></p>
						</div>
					</div>
				</div>
			</aside>
...
<nav class="fixed bottom-0 left-0 right-0 z-50 border-t border-stone-100 bg-white/90 backdrop-blur-md lg:hidden">
	<div class="flex items-center justify-around px-8 py-4">
		<a href="/" class="flex flex-col items-center gap-1.5 transition-colors {isActive('/') ? 'text-[#7C9A7E]' : 'text-stone-400'}">
			<Home size={24} strokeWidth={isActive('/') ? 2.5 : 2} />
			<span class="text-[10px] font-bold">홈</span>
		</a>
		<a href="/recipes" class="flex flex-col items-center gap-1.5 transition-colors {isActive('/recipes') ? 'text-[#7C9A7E]' : 'text-stone-400'}">
			<BookOpen size={24} strokeWidth={isActive('/recipes') ? 2.5 : 2} />
			<span class="text-[10px] font-bold">레시피</span>
		</a>
		<a href="/fridge" class="flex flex-col items-center gap-1.5 transition-colors {isActive('/fridge') ? 'text-[#7C9A7E]' : 'text-stone-400'}">
			<Refrigerator size={24} strokeWidth={isActive('/fridge') ? 2.5 : 2} />
			<span class="text-[10px] font-bold">냉장고</span>
		</a>
		<a href="/settings" class="flex flex-col items-center gap-1.5 transition-colors {isActive('/settings') ? 'text-[#7C9A7E]' : 'text-stone-400'}">
