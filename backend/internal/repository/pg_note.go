...
func (r *PgNoteRepository) FindByRecipeID(ctx context.Context, recipeID int) (*model.RecipeNote, error) {
	note := &model.RecipeNote{RecipeID: recipeID}
	err := r.pool.QueryRow(ctx,
		`SELECT memo, adjustments, updated_at FROM recipe_notes WHERE recipe_id = $1`,
		recipeID,
...
func (r *PgNoteRepository) Upsert(ctx context.Context, note *model.RecipeNote) error {
	_, err := r.pool.Exec(ctx, `
		INSERT INTO recipe_notes (recipe_id, memo, adjustments, updated_at)
		VALUES ($1, $2, $3, NOW())
		ON CONFLICT (recipe_id) DO UPDATE
		SET memo = EXCLUDED.memo,
		    adjustments = EXCLUDED.adjustments,
		    updated_at = NOW()
	`, note.RecipeID, note.Memo, note.Adjustments)
	return err
}
